...
def deletevisitor(request,pk):
    visitor = Visitor.objects.get(visitor_id=pk)
    visitdetails=VisitDetails.objects.get(visit_id=pk)
    if request.method == "POST":
        visitor.delete()
        visitdetails.delete()
        visitor = Visitor.objects.all().order_by('-visitor_id')
...
def updateevent(request,pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request,'basic/events.html',{'form':event,'complete':complete,'upcomming':upcomming,'k':False})
    context={'form':form,'k':False}
...
def accountsettings(request):
    ...
    if request.method=="POST":
        form=ProfileUpdateForm(request.POST,instance=request.user.host)
        if form.is_valid():
            form.save()
        ...
        form=ProfileUpdateForm(instance=request.user.host)
